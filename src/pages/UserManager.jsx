      ...
      >
        ...
        >
          <div>
            ...
            >
              {t('admin.generatedFor', { name: userName })}
            </p>
          </div>
          <button
            onClick={onClose}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isActive !== user.isActive)
        await api.patch(`/admin/users/${user.id}/status`, { isActive });
      if (isSuperAdmin !== user.isSuperAdmin)
        await api.patch(`/admin/users/${user.id}/promote`, { isSuperAdmin });

      toast.success(t('admin.userUpdated'));
      onSaved();
    } catch (err) {
      toast.error(translateServerError(err, t, 'admin.saveFailed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '24px 28px 0' }}>
          ...
          >
            <span>
              <b style={{ color: 'var(--foreground)' }}>{t('admin.idLabel')}</b>{' '}
              {user.publicId?.slice(0, 16)}…
            </span>
            <span>
              <b style={{ color: 'var(--foreground)' }}>{t('admin.created')}</b>{' '}
              {fmtDate(user.createdAt)}
            </span>
            <span>
              <b style={{ color: 'var(--foreground)' }}>
                {t('admin.lastLoginLabel')}
              </b>{' '}
...
export default function UserManager() {
  ...
  const fetchUsers = useCallback(
    async (q = search, p = page, s = statusFilter, r = roleFilter) => {
      setLoading(true);
      try {
    ...
    [search, page, statusFilter, roleFilter, t],
  );
  ...
  const handleSearchChange = (val) => {
    ...
    searchTimer.current = setTimeout(() => {
      setPage(1);
      fetchUsers(val, 1, statusFilter, roleFilter);
    }, 400);
  };
  ...
  const handleResetConfirm = async () => {
    ...
    try {
      ...
      toast.error(translateServerError(err, t, 'admin.resetFailed'));
    } finally {
      setResetting(false);
      setResetTarget(null);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t('admin.deletedSuccess', { name: deleteTarget.name }));
      setDeleteTarget(null);
      setPage(1);
      fetchUsers(search, 1, statusFilter, roleFilter);
    } catch (err) {
      toast.error(translateServerError(err, t, 'admin.deletionFailed'));
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p
          style={{ margin: 0, color: 'var(--muted-foreground)', fontSize: 14 }}
        >
          {t('admin.usersRegistered', { count: total })}
        </p>
      </div>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button
                          ...
                          title={t('admin.resetPasswordTitleAttr')}
                          style={{
                            display: 'flex',
                            alignItems: 'center',
                            padding: '6px 8px',
        ...
        {pages > 1 && (
          ...
          >
            <span style={{ fontSize: 13, color: 'var(--muted-foreground)' }}>
              {t('admin.pageOf', { page, pages, total })}
            </span>
      ...
      <ConfirmDialog
        ...
        message={t('admin.resetMessage', { name: resetTarget?.name })}
        confirmLabel={
          resetting ? t('admin.resetting') : t('admin.resetPasswordBtn')
        }
      ...
      <ConfirmDialog
        ...
        danger
        message={t('admin.deleteMessage', { name: deleteTarget?.name })}
        confirmLabel={
          deleting ? t('admin.deleting') : t('admin.deleteConfirmBtn')
        }
      ...
      >
        <div>
          ...
          >
            {t('admin.typeNameConfirm')}{' '}
            <b style={{ color: 'var(--foreground)' }}>{deleteTarget?.name}</b>
          </p>
          <input
            ...
            style={{
              ...
              padding: '9px 14px',
              borderRadius: 8,
              border: `1px solid ${deleteConfirmText === deleteTarget?.name ? '#ef4444' : 'var(--border)'}`,
              background: 'var(--background)',
              color: 'var(--foreground)',
              fontSize: 14,
              boxSizing: 'border-box',
